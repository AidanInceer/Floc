...
export type CandidateRun = {
  ...
  nights: number;
  /** Everyone free on *every* day of the run — the people it actually suits. */
  going: string[];
  /** Members who can't do at least one day of it, so the group knows who to chase. */
  missing: string[];
};

/**
 * The runs of days worth arguing about (ticket 197). One run per attendance
 * level, from "everyone" downwards: at each level, the longest stretch where
 * every day clears it. Runs nobody could improve on are dropped, so the list
 * stays short enough to read rather than exhaustive.
 *
 * Deliberately not the same call as `bestWindow`: that answers "what should the
 * dates be", this answers "what are the options", and the shortlist has to show
 * the ones that lose someone as well as the one that doesn't.
 */
export function candidateRuns(
  rows: AvailabilityRow[],
  memberIds: string[],
  { minNights = 1, limit = 5 }: { minNights?: number; limit?: number } = {},
): CandidateRun[] {
  const counts = tally(rows);
  const dates = [...counts.keys()].sort();
  if (dates.length === 0) return [];

  const freeOn = new Map<IsoDate, Set<string>>();
  for (const row of rows) {
    if (!row.available) continue;
    const set = freeOn.get(row.date) ?? new Set<string>();
    set.add(row.userId);
    freeOn.set(row.date, set);
  }
  ...
  for (let threshold = top; threshold >= 1; threshold -= 1) {
    const run = longestRun(dates, counts, threshold);
    if (!run) continue;
    const key = `${run.start}:${run.end}`;
    if (seen.has(key)) continue;
    seen.add(key);

    const nights = runLength(run) - 1;
    if (nights < minNights) continue;

    // Whoever is free on every single day — an intersection, not a sum, since a
    // run someone can only half-do isn't a run they can do.
    let going = [...(freeOn.get(run.start) ?? [])];
    for (let d = run.start; d !== run.end; d = addDays(d, 1)) {
      const next = freeOn.get(addDays(d, 1)) ?? new Set<string>();
      going = going.filter((u) => next.has(u));
    }
    ...
    out.push({
      ...
      nights,
      going,
      missing: memberIds.filter((m) => !going.includes(m)),
    });
  }
