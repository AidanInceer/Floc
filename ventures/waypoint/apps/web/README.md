...
src/
  ...
  lib/                    access, auth, dates, money, unlocks, email,
                          ...
                          travel-map + countries (the profile's country map)
...
| A travel map mark from a trip is never stored | `lib/travel-map.ts` — `user_country_mark` holds hand-painted rows only; trip marks are derived on read, because "been there" is triggered by time passing (ticket 95) |
